...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PiecesofEight: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "PiecesofEight Signed Message:\n";
...
{
    // PiecesofEight: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    int64 nSubsidy = 800 * COIN;

    // Subsidy is cut in half every 5,000,000 blocks, which will occur approximately every 12 years
    nSubsidy >>= (nHeight / 5000000); // PiecesofEight: 800k blocks in ~12 years 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 288000; // PiecesofEight: 80 hours (80 * 60 * 60)
static const int64 nTargetSpacing = 80; // PiecesofEight: 80 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PiecesofEight: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PiecesofEight: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PiecesofEight: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "January 4 2014 - Headline goes here";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // PiecesofEight: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PiecesofEightMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PiecesofEightMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PiecesofEightMiner(CWallet *pwallet)
{
    printf("PiecesofEightMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("PiecesofEight-miner");
    ...
    try { loop {
        ...
        printf("Running PiecesofEightMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PiecesofEightMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PiecesofEightMiner, pwallet));
}
